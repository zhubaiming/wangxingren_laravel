...
enum ResponseEnum
{
    // 001 ~ 099 表示系统状态；100 ~ 199 表示授权业务；200~ 299表示用户业务

    /*--------------*/
    // 100 开头的表示【信息提示】，这类状态表示临时的响应
    // 100 - 继续
    // 101 - 切换协议
    /*----------*/
    ...
    const USER_SERVICE_LOGIN_ERROR = [200201, '登录失败: 账号或密码错误'];
    const USER_SERVICE_LOGIN_STATUS_ERROR = [200202, '登录失败: 账号不可登录'];
    const USER_SERVICE_LOGIN_CAPTCHA_ERROR = [200202, '登录失败: 验证码错误'];
    ...
    /*----------*/
    // 300 开头的表示服务器重定向，指向的别的地方，客户端浏览器必须采取更多操作来实现请求
    // 302 - 对象已移动
    // 304 - 未修改
    // 307 - 临时重定向
    /*----------*/
    // 400 开头的表示客户端请求错误，请求不到数据或者找不到等等
    // 400 - 错误的请求
    ...
    // 423 - 锁定的错误
    /*----------*/
    // 500 开头的白哦时服务器错误，服务器因为代码，或者什么原因终止执行
